...
,r[8*t+4]=.01//scale
,r[8*t+5]=.3*Math.random(),r[8*t+6]=.7*Math.random(),r[8*t+7]=.3*Math.random();return r},i=(e,r)=>e.createBuffer({label:"work buffer",size:r,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_SRC|GPUBufferUsage.COPY_DST}),o=(e,r)=>e.createBuffer({label:"result buffer",size:r,usage:GPUBufferUsage.MAP_READ|GPUBufferUsage.COPY_DST}),u=(e,r,t,a,n)=>e.createBindGroup({layout:r.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:t}},{binding:1,resource:{buffer:a}},{binding:2,resource:{buffer:n}}]}),l=(e,r,t)=>e.createBindGroup({layout:r.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:t}}]}),s=0,d=0,v=async(e,r,t,a,n,c,i,o)=>{let u=d-s;s=d,e.queue.writeBuffer(r,0,new Float32Array([.05*u]));let l=e.createCommandEncoder({label:"doubling encoder"}),f=l.beginComputePass({label:"doubling compute pass"});f.setPipeline(t),f.setBindGroup(0,a[d%2].bindGroup),f.dispatchWorkgroups(Math.ceil(n/64)),f.end(),l.copyBufferToBuffer(a[d%2].buffer,0,c,0,c.size),e.queue.submit([l.finish()]),// Read the results
